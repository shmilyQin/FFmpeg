...
int main(int argc, char **argv)
{
    ...
    {
        if (firstback)
            avformat_seek_file(ic, -1, INT64_MIN, seekfirst, seekfirst, AVSEEK_FLAG_BACKWARD);
        else
            avformat_seek_file(ic, -1, seekfirst, seekfirst, INT64_MAX, AVSEEK_FLAG_BACKWARD);
    }
    for (i = 0;; i++)
    {
        AVPacket pkt = {0};
        ...
        if (i & 1)
            ret = avformat_seek_file(ic, stream_id, INT64_MIN, timestamp, timestamp, AVSEEK_FLAG_BACKWARD);
        else
            ret = avformat_seek_file(ic, stream_id, timestamp, timestamp, INT64_MAX, AVSEEK_FLAG_BACKWARD);
