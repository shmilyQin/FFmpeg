...
static int open_file(AVFormatContext *avf, unsigned fileno)
{
    ...
    if (file->start_time == AV_NOPTS_VALUE)
        file->start_time = !fileno ? 0 : cat->files[fileno - 1].start_time + cat->files[fileno - 1].duration;
    ...
    {
        if ((ret = avformat_seek_file(cat->avf, -1, INT64_MIN, file->inpoint, file->inpoint, AVSEEK_FLAG_BACKWARD)) < 0)
            return ret;
    }
    return 0;
}
...
static const AVOption options[] = {
    {"safe", "enable safe mode", OFFSET(safe), AV_OPT_TYPE_BOOL, {.i64 = 1}, -1, 1, DEC},
    {"auto_convert", "automatically convert bitstream format", OFFSET(auto_convert), AV_OPT_TYPE_BOOL, {.i64 = 1}, 0, 1, DEC},
    {"segment_time_metadata", "output file segment start time and duration as packet metadata", OFFSET(segment_time_metadata), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, DEC},
    {NULL}};
