...
static struct section sections[] = {
    ...
    [SECTION_ID_ROOT] = {SECTION_ID_ROOT, "root", SECTION_FLAG_IS_WRAPPER, {SECTION_ID_CHAPTERS, SECTION_ID_FORMAT, SECTION_ID_FRAMES, SECTION_ID_PROGRAMS, SECTION_ID_STREAMS, SECTION_ID_PACKETS, SECTION_ID_ERROR, SECTION_ID_PROGRAM_VERSION, SECTION_ID_LIBRARY_VERSIONS, SECTION_ID_PIXEL_FORMATS, -1}},
...
static const AVOption writer_options[] = {
    {"string_validation", "set string validation mode", OFFSET(string_validation), AV_OPT_TYPE_INT, {.i64 = WRITER_STRING_VALIDATION_REPLACE}, 0, WRITER_STRING_VALIDATION_NB - 1, .unit = "sv"},
                                      ...
                                      const char *key, const char *val, int flags)
{
    ...
    if ((flags & PRINT_STRING_OPT) && !(wctx->writer->flags & WRITER_FLAG_DISPLAY_OPTIONAL_FIELDS))
        return 0;
                       ...
                       AVFormatContext *fmt_ctx)
{
    ...
    if (av_frame_get_pkt_pos(frame) != -1)
        print_fmt("pkt_pos", "%" PRId64, av_frame_get_pkt_pos(frame));
    else
        print_str_opt("pkt_pos", "N/A");
    if (av_frame_get_pkt_size(frame) != -1)
        print_val("pkt_size", av_frame_get_pkt_size(frame), unit_byte_str);
    else
                                 ...
                                 const ReadInterval *interval, int64_t *cur_ts)
{
    ...
    if (interval->has_start)
    {
        ...
        if ((ret = avformat_seek_file(fmt_ctx, -1, -INT64_MAX, target, INT64_MAX, AVSEEK_FLAG_BACKWARD)) < 0)
        {
...
#include "cmdutils_common_opts.h"
    ...
    {"byte_binary_prefix", OPT_BOOL, {&use_byte_value_binary_prefix}, "use binary prefixes for byte units"},
    ...
    {"print_format", OPT_STRING | HAS_ARG, {(void *)&print_format}, "set the output printing format (available formats are: default, compact, csv, flat, ini, json, xml)", "format"},
    ...
    {"show_format_entry", HAS_ARG, {.func_arg = opt_show_format_entry}, "show a particular entry from the format/container info", "entry"},
    {"show_entries", HAS_ARG, {.func_arg = opt_show_entries}, "show a set of specified entries", "entry_list"},
